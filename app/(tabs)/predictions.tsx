...
const STATUS_STYLE: Record<PredictionStatus, { bg: string; text: string; label: string }> = {
  ...
  VOID:     { bg: "rgba(115,115,115,0.1)", text: "#737373", label: "Hủy" },
};

function PredictionCard({ p }: { p: Prediction }) {
  const s = STATUS_STYLE[p.status];
  return (
    <View style={{ backgroundColor: "#171717", borderWidth: 1, borderColor: "#262626", borderRadius: 16, padding: 16, marginBottom: 10 }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 8 }}>
        <Text style={{ color: "#737373", fontSize: 11 }}>{p.matchSnapshot.leagueName}</Text>
      ...
      <Text style={{ color: "#fff", fontWeight: "600", fontSize: 14, marginBottom: 12 }}>
        {p.matchSnapshot.homeTeam.name} vs {p.matchSnapshot.awayTeam.name}
      </Text>
      <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
        <View>
          <Text style={{ color: "#737373", fontSize: 11 }}>Dự đoán</Text>
          <Text style={{ color: "#fff", fontSize: 13, fontWeight: "500", marginTop: 2 }}>
            {p.outcome.replace("_", " ")}
          </Text>
        </View>
        <View style={{ alignItems: "center" }}>
          <Text style={{ color: "#737373", fontSize: 11 }}>Tỷ lệ</Text>
          <Text style={{ color: "#fff", fontSize: 13, fontWeight: "500", marginTop: 2 }}>
        ...
        <View style={{ alignItems: "flex-end" }}>
          ...
          <Text style={{ color: p.status === "WON" ? "#14b8a6" : p.status === "LOST" ? "#ef4444" : "#fff", fontSize: 14, fontWeight: "700", marginTop: 2 }}>
            {p.status === "WON"
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#0a0a0a" }}>
      ...
      <View style={{ paddingHorizontal: 16, paddingTop: 16, paddingBottom: 8, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        <View>
          <Text style={{ color: "#fff", fontSize: 24, fontWeight: "700" }}>Dự đoán</Text>
          <Text style={{ color: "#737373", fontSize: 13, marginTop: 2 }}>🪙 {balance.toLocaleString()} coin</Text>
        </View>
      </View>
        ...
        {TABS.map(({ key, label }) => (
          <TouchableOpacity
            ...
            style={{
              ...
              backgroundColor: tab === key ? "#14b8a6" : "#171717",
              borderWidth: 1, borderColor: tab === key ? "#14b8a6" : "#262626",
            }}
          >
            <Text style={{ color: tab === key ? "#fff" : "#737373", fontSize: 13, fontWeight: "600" }}>
              {label}
            </Text>
          </TouchableOpacity>
        ))}
          ...
          <View>
              ...
              {([ ["allTime", "Toàn thời gian"], ["monthly", "Tháng này"], ["weekly", "Tuần này"] ] as const).map(([k, label]) => (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    backgroundColor: periodKey === k ? "rgba(20,184,166,0.15)" : "#171717",
                    borderWidth: 1, borderColor: periodKey === k ? "#14b8a6" : "#262626",
                  }}
                >
                  <Text style={{ color: periodKey === k ? "#14b8a6" : "#737373", fontSize: 12, fontWeight: "600" }}>
                    {label}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            {myRank && (
              <View style={{ backgroundColor: "rgba(20,184,166,0.1)", borderWidth: 1, borderColor: "rgba(20,184,166,0.3)", borderRadius: 16, padding: 16, marginBottom: 12 }}>
                <Text style={{ color: "#737373", fontSize: 12, marginBottom: 6 }}>Hạng của bạn</Text>
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                  <Text style={{ color: "#fff", fontSize: 24, fontWeight: "700" }}>{getRankEmoji(myRank.rank)}</Text>
                  <View style={{ alignItems: "flex-end" }}>
                    <Text style={{ color: "#14b8a6", fontSize: 16, fontWeight: "700" }}>{formatScore(myRank.score)} điểm</Text>
                    <Text style={{ color: "#737373", fontSize: 12, marginTop: 2 }}>{Math.round(myRank.winRate * 100)}% thắng</Text>
                  </View>
                </View>
              </View>
            )}
            ...
            {boardLoad ? <LoadingSpinner /> : (
              <View style={{ gap: 8 }}>
                {(board?.topEntries ?? []).map((entry) => (
                  ...
                  >
                    <Text style={{ color: "#fff", fontWeight: "700", fontSize: 16, width: 36, textAlign: "center" }}>
